...
constexpr float aspectRatio = 1; // 16.f / 9.f;
...
#pragma warning (pop)
	...
	vec3 operator * ( const vec3& operand ) const { return vec3( x * operand.x, y * operand.y, z * operand.z ); }
	vec3 operator / (const vec3& operand) const { return vec3(x / operand.x, y / operand.y, z / operand.z); }
	vec3 operator / (const int a) const { return vec3(x / a, y / a, z / a); }
	void operator -= ( const vec3& a ) { x -= a.x; y -= a.y; z -= a.z; }
	void operator += ( const vec3& a ) { x += a.x; y += a.y; z += a.z; }
	void operator *= ( const vec3& a ) { x *= a.x; y *= a.y; z *= a.z; }
	void operator *= ( const float a ) { x *= a; y *= a; z *= a; }
	bool operator < (const vec3& operand) { return x < operand.x && y < operand.y && z < operand.z; }
	bool operator <= (const vec3& operand) { return x <= operand.x&& y <= operand.y&& z <= operand.z; }
	bool operator > (const vec3& operand) { return x > operand.x && y > operand.y && z > operand.z; }
	bool operator >= (const vec3& operand) { return x <= operand.x && y <= operand.y && z <= operand.z; }
	...
	vec3 reverse() const { return vec3(1.f / x, 1.f / y, 1.f / z); }
	...
	vec3 invert() { return vec3(1.f / x, 1.f / y, 1.f / z); }
	...
	vec3 ReflectRay(vec3 R, vec3 N) const { return  N * 2 * N.dot(R) - R; }
	vec3 RefractRay(const vec3& uv, const vec3& n, float etai_over_etat) const
	{
		const auto cos_theta = fminf(n.dot(-uv), 1.f);
		const vec3 r_out_perpendicular = (uv + n * cos_theta) * etai_over_etat;
		const vec3 r_out_parallel = n * -sqrtf(fabsf(1.f - r_out_perpendicular.sqrLentgh()));
...
inline 	float max_component(vec3 a) {return Max(Max(a.x, a.y), a.z);}
inline 	float min_component(vec3 a) { return Min(Min(a.x, a.y), a.z); }
inline vec3 pairwise_min(vec3 a, vec3 b) { return vec3(Min(a.x, b.x), Min(a.y, b.y), Min(a.z, b.z)); }
inline vec3 pairwise_max(vec3 a, vec3 b) { return vec3(Max(a.x, b.x), Max(a.y, b.y), Max(a.z, b.z)); }
